...
## 3.5+ ポジティブ感情（うれしさ・驚き）の評価

自動車HMIのUXは、負荷やストレスの低減（＝ネガティブの抑制）だけでなく、
**「うれしさ・満足・驚き・ワクワク」といったポジティブな感情体験の喚起**によっても
高まります。車内感情の検出と調整はUXと安全の双方に寄与します
（Braun 2021 [31]、Zepf 2020 [32]）。ここでは感情を **感情価（valence: 快−不快）** と
**覚醒（arousal: 高−低）** の2次元で捉えます（Höfling 2020 [33]）。

### 3.5+.1 うれしさ・満足（正の感情価）
- **大頬骨筋（zygomaticus, 笑筋）の活動上昇**がポジティブな感情価と正相関する
  （Hazlett & Benedek 2007 [34]、Partala 2006 [35]、Wang & Song 2021 [38]）。
- HMI操作が「楽しい・心地よい」ほど、この笑筋活動が増える。UXの魅力（hedonic quality）を
  生理的に捉える中核サイン。
- **負担の少ない計測**: メガネ型など低装着感ウェアラブルEMG（Gjoreski 2022 [36]）、
  または**カメラ表情解析（Action Unit 6/12＝頬上げ・口角上げ）で非接触に代替**
  （Höfling 2020 [33]、Di Tecco 2025 [37]）。

### 3.5+.2 驚き（surprise）
- **覚醒の急上昇**（EDAの位相性反応・瞬間的な心拍変化）と、**眉上げ・目の見開き**
  （表情AU1/2/5）で特徴づけられる（Höfling 2020 [33]、Zepf 2020 [32]）。
- 表情認識研究では**「驚き」が検出表情の大半を占める**ほど顕著に現れやすい
  （Di Tecco 2025 [37]）。ポジティブな驚き（嬉しいサプライズ）とネガティブな驚き
  （警告への驚愕）は、**同時計測する感情価（笑筋 vs 皺眉筋）で判別**する。
- **計測**: EDAの位相性SCR（覚醒の急峻な立ち上がり）＋カメラ表情解析（AU1/2/5）で非接触評価。

### 3.5+.3 興奮・エンゲージメント
- **高覚醒＋正の感情価**の組み合わせ＝ワクワク・没入。EDA上昇と笑筋EMGの同時上昇で捉える
  （Akan & Berkman 2020 [28]、Wang & Song 2021 [38]）。

### 3.5+.4 感情の2次元マッピング（解釈の枠組み）
| 感情 | 感情価 | 覚醒 | 主な計測サイン | 根拠 |
|------|-------|------|--------------|------|
| うれしさ/満足 | 高（快） | 中 | 大頬骨筋EMG↑ / AU6・12 | [34][35][36][38] |
| 嬉しい驚き | 高（快） | 高 | EDA急上昇＋笑筋↑＋AU1/2/5 | [32][33][37] |
| ネガティブな驚き/驚愕 | 低（不快） | 高 | EDA急上昇＋皺眉筋↑＋AU4 | [32][33] |
| 不快/フラストレーション | 低（不快） | 中 | 皺眉筋EMG↑ / AU4 | [24][33] |

> **設計への含意**: 良好なHMIは、ネガティブ感情（負荷・苛立ち）を抑えつつ、
> 適切な場面でポジティブな驚き・うれしさを喚起する。感情価（笑筋/表情）と覚醒（EDA）を
> 併せて測ることで、「単なる無ストレス」と「積極的に良い体験」を区別できる（[31][33]）。
...
4. **混合プロトタイピング**: 実車前にVR/モックアップで安価に体験を再現し早期評価
   （Peruzzini 2018 [01]）。
5. **ポジティブ感情の喚起シナリオ**: 気の利いた提案・演出・サプライズ機能など、
   うれしさ・驚きを引き出す場面を用意し、感情価×覚醒の反応を評価
   （Braun 2021 [31]、Di Tecco 2025 [37]）。
...
| 大頬骨筋(笑筋)EMG上昇・AU6/12 | うれしさ・満足（正の感情価＝良好UX） | [34][35][36][38] |
| 皺眉筋EMG上昇・AU4 | 不快・フラストレーション | [24][33] |
| EDA急上昇＋眉上げ/目見開き(AU1/2/5) | 驚き（感情価は笑筋/皺眉筋で判別） | [32][33][37] |
| EDA上昇＋笑筋EMG上昇 | 興奮・エンゲージメント（ワクワク） | [28][38] |
| 主観(SUS/NASA-TLX)と生理が一致 | 結論の妥当性が高い | [09][26] |

> 良好な自動車HMIとは、**同一タスクを、より低い生理的負荷（HRV維持・EDA安定）、
> より短い視覚的探索、より高い主観満足で完了できる**設計です（[05][12][16]）。
> さらに一段上のHMIは、**適切な場面でうれしさ・ポジティブな驚きを喚起し、感情価（笑筋/表情）
> と覚醒（EDA）の両面で良い反応を引き出せる**設計です（[31][33][37]）。
...
| 表情・情動価 | **カメラ表情解析（非接触, FACS/AU推定：AU6/12＝喜び, AU1/2/5＝驚き, AU4＝不快）** で表情筋EMGを代替 | 顔面表面EMG／メガネ型ウェアラブルEMG（精査時, [36]） | 動画 ≥ 30 fps／EMG ≥ 1000 Hz | 1（代替）／2〜3 | [21][24][28][33][34][36][37] |
...
- **感情価/表情**: カメラ表情解析でAction Unit（AU6/12＝笑顔, AU4＝しかめ面, AU1/2/5＝驚き）を
  抽出。大頬骨筋−皺眉筋の差分を感情価スコアに（Höfling 2020 [33]、[34][35][37]）。
...
6. **ネガティブ抑制だけでなくポジティブ感情（うれしさ・驚き）も評価**。感情価（笑筋/表情AU）
   ×覚醒（EDA）の2次元で、「無ストレス」と「積極的に良い体験」を区別（[31][33][34][37]）。
...
| [31] | Braun 2021 | 車内感情の検出・調整、感情的自動車UI |
| [32] | Zepf 2020 | ドライバー感情認識、驚きを含む多面計測 |
| [33] | Höfling 2020 | 感情価=表情筋/表情、覚醒=EDAの役割分担、非接触代替 |
| [34] | Hazlett & Benedek 2007 | 大頬骨筋EMGで正の感情価(UX)を計測 |
| [35] | Partala 2006 | 表情から感情価をリアルタイム推定 |
| [36] | Gjoreski 2022 | 低装着感ウェアラブル表情筋EMG |
| [37] | Di Tecco 2025 | 表情認識→UX、驚きの顕著性 |
| [38] | Wang & Song 2021 | 笑筋=興奮/幸福、感情的体験設計 |

> 各文献の詳細は [00_MASTER_SUMMARY_all_references.md](00_MASTER_SUMMARY_all_references.md)、
> 各個別ファイル（01〜29）、および感情評価は
> [31-38_Emotion_PositiveAffect_Surprise_References.md](31-38_Emotion_PositiveAffect_Surprise_References.md)
> を参照。本提案は各文献の要旨に基づき構成しており、
